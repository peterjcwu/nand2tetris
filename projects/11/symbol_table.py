...
class SymbolTable:
    ...
    and since no "linking" needs to be done by the compiler, these identifiers do not
    have to be kept in the symbol table.
    """
    ...
    def define(self, name, t, kind):
        ...
        if kind in ['static', 'field']:
            self._class[name] = {'type': t, 'kind': kind, 'index': self._var_count[kind]}

        else:
            self._subroutine[name] = {'type': t, 'kind': kind, 'index': self._var_count[kind]}
    ...
    def exists(self, name) -> bool:
        ...
        return name in self._subroutine or name in self._class
    ...
    def index_of(self, name):
        """ Returns the index, as integer, assigned to named identifier """
        if name in self._subroutine:

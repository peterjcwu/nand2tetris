...
class CompilationEngine:
    ...
    def __init__(self, input_full_path: str, output_full_path: str):
        """
        ...
        self._file_name = os.path.basename(input_full_path)
    ...
    def compile_subroutine(self):
        ...
        if is_method:      
            ...
            num_args = int(self._var_table.var_count('field'))
    ...
    def compile_subroutine_call(self):
        ...
        if self._tokenize.get_current_token() == '.':
            ...
            if self._var_table.exists(class_name):
                ...
                code = vm.write_push(kind, int(index))
                self.write(code)                 
        ...
        code = vm.write_call(class_name, func_name, int(num_args))
        self.write(code)        
    ...
    def compile_let(self):
        ...
        if is_array:    
            ...
            code = vm.write_pop(kind, int(index))
    ...
    def compile_term(self):
        ...
        # this is a unaryOp followed by a term
            ...
            self.eat(['-', '~'])
            self.compile_term()
            code = vm.write_arithmetic(unary_op)
            self.write(code)       
    ...
    def eat(self, terminal):
        ...
        # when a token is a keyword or symbol, it should match the terminal or a terminal option,
        # input as a string or list
        ...
        # should match an item in ['className', 'subroutineName', 'varName']
            ...
            error = ('\nERROR 4: terminal ' + str(terminal)
                     + ' has no match in [\'className\', \'subroutineName\', \'varName\'] '
                     + ' from file: ' + self._file_name) 
            self.print_error(error)
